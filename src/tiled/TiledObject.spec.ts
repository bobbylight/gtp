...
describe('TiledObject', () => {
	...
	describe('intersects()', () => {
		...
		it('returns success when top-left corner shared and completely contained', () => {
			expect(intersects(object, 0, 0, 1, 1)).toEqual(true);
		});

		it('returns success when exact match checked', () => {
			expect(intersects(object, 0, 0, 5, 5)).toEqual(true);
		});

		it('returns success when other rect completely contained', () => {
			expect(intersects(object, 1, 1, 1, 1)).toEqual(true);
		});

		it('returns success when top-left corner contained', () => {
			expect(intersects(object, 3, 3, 5, 5)).toEqual(true);
		});

		it('returns success when bottom-right corner contained', () => {
			expect(intersects(object, -2, -2, 3, 3)).toEqual(true);
		});

		it('returns false if other rect is outside', () => {
			expect(intersects(object, 5, 5, 1, 1)).toEqual(false);
		});

		it('returns false if width === 0', () => {
			object.width = 0;
			expect(intersects(object, 1, 1, 1, 1)).toEqual(false);
		});
		...
		it('returns false if other width === 0', () => {
			expect(intersects(object, 1, 1, 0, 1)).toEqual(false);
		});
		...
		it('returns false if other height === 0', () => {
			expect(intersects(object, 1, 1, 1, 0)).toEqual(false);
		});
